...
- [ ] Milestone 3: Create a simple ESP32 serial source
  ...
  - [ ] Print at least one valid JSON line from the ESP32.
  - [x] Print at least one non-JSON diagnostic line from the ESP32. Verified with simple-mesh boot/test-message logs from COM18.
  - [x] Verify SGC can handle mixed logs and JSON. Verified local JSON samples and real non-JSON firmware logs.
...
- [ ] Milestone 4: Validate browser/device behavior
  ...
  - [ ] Document any smartphone/tablet limitation found during the test.
...
- [ ] Milestone 5: Define pass/fail criteria
  ...
  - [x] Pass if SGC can parse valid JSON lines. Verified with local parser sample buttons.
